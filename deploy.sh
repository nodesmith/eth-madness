...
kubectl config use-context aws-us-east --kubeconfig $TRAVIS_BUILD_DIR/kubeconfig

PATCH_TEMPLATE=$'\'{"spec": {"template": {"spec": {"containers":[{"name":"app","image":"IMAGE_NAME"}]}}}}\''
PATCH=$(echo ${PATCH_TEMPLATE/IMAGE_NAME/$IMAGE_NAME})
CMD="kubectl patch deployment eth-madness --kubeconfig $TRAVIS_BUILD_DIR/kubeconfig -p $PATCH"
eval $CMD
